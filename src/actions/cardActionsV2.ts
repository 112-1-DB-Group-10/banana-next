...
const getInstituteByUserId = async (userId: UUID): Promise<string> => {
  ...
  const institute =
    institutePackage.length == 0 ? '' : institutePackage[0].institute;
  return institute;
};
...
const getPopularCards = async (
  ...
  if (locations.length == 0) {
    console.log('nothing');
    cardsInLocation = db.$with('cardsInLocation').as(
      db
        .select({
    ...
    cardsInLocation = db.$with('cardsInLocation').as(
      db
        ...
        .innerJoin(
          cardIdsInLocation,
          eq(cardsTable.card_id, cardIdsInLocation.card_id),
        ),
    );
  }
  ...
  const cards = await db
    ...
    .leftJoin(
      commentsSubquery,
      eq(commentsSubquery.card_id, cardsInLocation.card_id),
    )
    ...
    .leftJoin(
      wantToLearnTable,
      eq(wantToLearnTable.card_id, cardsInLocation.card_id),
    )
    ...
    .orderBy(
      desc(
        sql<number>`coalesce(${commentsSubquery.commentCount}, 0) * 3 + coalesce(${likesSubquery.likeCount}, 0)`,
      ),
    )
    .limit(cardPerPage)
...
const getCardsBySubstring = async (
  verifiedUser: boolean,
  mysubstring: string,
  locations: Array<string>,
  cardPerPage: number,
...
const getCardsByLabel = async (
  ...
  let cards = await db
    ...
    .innerJoin(
      cardIdsInLabel,
      eq(cardIdsInLabel.card_id, cardsInLocation.card_id),
    )
    .leftJoin(
      commentsSubquery,
      eq(commentsSubquery.card_id, cardsInLocation.card_id),
...
const getCardsByTopic = async (
  ...
  const cardIdsInLabels = db
    ...
    .innerJoin(
      labelsInTopic,
      eq(goodAtTable.label_name, labelsInTopic.label_name),
    )
    .as('cardIDInLabels');
  ...
  let cards = await db
    ...
    .innerJoin(
      cardIdsInLabels,
      eq(cardIdsInLabels.card_id, cardsInLocation.card_id),
    )
    .leftJoin(
      commentsSubquery,
      eq(commentsSubquery.card_id, cardsInLocation.card_id),
...
const getCardsPostedByUser = async (
  userId: UUID,
  cardPerPage: number,
  page: number,
): Promise<CardData[]> => {
  ...
  const cards = await db
    ...
    .leftJoin(
      commentsSubquery,
      eq(commentsSubquery.card_id, cardsTable.card_id),
    )
    ...
    .leftJoin(
      wantToLearnTable,
      eq(wantToLearnTable.card_id, cardsTable.card_id),
    )
...
const getCardsLikedOrCommentedByUser = async (
  userId: UUID,
  cardPerPage: number,
  page: number,
): Promise<CardData[]> => {
